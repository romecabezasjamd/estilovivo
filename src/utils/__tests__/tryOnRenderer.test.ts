...
describe('calculateGarmentTransform', () => {
  it('returns correct transform for top garments', () => {
    ...
    expect(t.scaleX).toBeCloseTo((200 * 1.22) / 200, 2)
    expect(t.scaleY).toBeCloseTo((220 * 1.22) / 200, 2)
  })

  it('returns correct transform for bottom garments', () => {
    const t = calculateGarmentTransform(defaultDims, 'pants')
    expect(t.scaleX).toBeCloseTo(Math.max(180, 160) * 1.18 / 200, 2)
  ...
  it('returns correct transform for dress', () => {
    ...
    expect(t.scaleX).toBeCloseTo(Math.max(200, 180) * 1.22 / 200, 2)
    expect(t.scaleY).toBeCloseTo((220 + 260 * 0.88) / 200, 2)
  })

  it('returns correct transform for outerwear', () => {
    const t = calculateGarmentTransform(defaultDims, 'jacket')
    expect(t.scaleX).toBeCloseTo((200 * 1.28) / 200, 2)
    expect(t.scaleY).toBeCloseTo((220 * 1.35) / 200, 2)
  })

  it('returns correct transform for shoes', () => {
    const t = calculateGarmentTransform(defaultDims, 'shoes')
    expect(t.y).toBe(defaultDims.imageHeight - 55)
  ...
  it('returns correct transform for accessories', () => {
    ...
    expect(t.scaleX).toBeCloseTo((200 * 0.44) / 200, 2)
  })
  ...
  it('handles different body proportions', () => {
    ...
    expect(t.scaleX).toBeCloseTo((300 * 1.22) / 200, 2)
  })

  it('handles blouse as top', () => {
    const t = calculateGarmentTransform(defaultDims, 'blouse')
    expect(t.scaleX).toBeCloseTo((200 * 1.22) / 200, 2)
  })

  it('handles skirt as bottom', () => {
    const t = calculateGarmentTransform(defaultDims, 'skirt')
    expect(t.scaleX).toBeCloseTo(Math.max(180, 160) * 1.18 / 200, 2)
  })

  it('handles coat as outerwear', () => {
    const t = calculateGarmentTransform(defaultDims, 'coat')
    expect(t.scaleX).toBeCloseTo((200 * 1.28) / 200, 2)
  })
})
